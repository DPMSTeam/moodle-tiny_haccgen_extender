...
export const insertAudioHtml = (editor, url) => {
  const safe = escapeHtml(normalizeMediaUrl(url));
  editor.focus();
  moveSelectionToEnd(editor);
  editor.insertContent(`<p><audio controls preload="metadata" style="width:100%;max-width:100%;" src="${safe}"></audio></p>`);
};

export const replaceWithAudioHtml = (editor, url) => {
  const safe = escapeHtml(normalizeMediaUrl(url));
  editor.selection.setContent(`<audio controls preload="metadata" style="width:100%;max-width:100%;" src="${safe}"></audio>`);
};
